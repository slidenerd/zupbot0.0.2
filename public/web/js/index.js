...
function insertMessage() {
  ...
  if (text != null && text != '') {
      ...
      if (emails != null && emails != '') {
          ...
          var mailchimp = {};
          mailchimp.dc = 'us9';
          mailchimp.listid = '99ae52ebd6';
          mailchimp.username = 'slidenerd';
          mailchimp.password = 'a6b0097fc5ad9a0997df380ba1d9fa10-us9';
          var url = 'https://us9.api.mailchimp.com/2.0/lists/subscribe.json?apikey='+ mailchimp.password +'&id='+ mailchimp.listid;
          var sendData = {
              "email_address": emails[0]
          };
          
          if (sendData != '' && sendData != null) {
              var params = JSON.stringify(sendData);
              console.log(params);
              $.ajax({
                  url: url,
                  method: 'POST',
                  beforeSend: function(xhr) { xhr.setRequestHeader("Authorization", "Basic " + btoa(mailchimp.username + ":" + mailchimp.password)); },
                  data: params,
                  dataType: 'jsonp',
